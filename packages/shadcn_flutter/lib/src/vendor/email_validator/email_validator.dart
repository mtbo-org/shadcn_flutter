// Kept byte-for-byte close to upstream so it stays easy to diff against a
// newer release; upstream's enum naming and doc coverage are not ours to
// change.
// ignore_for_file: constant_identifier_names, public_member_api_docs

/// RFC-shaped email address validation, vendored from `package:email_validator`.
///
/// shadcn_flutter used to depend on `email_validator` for a single [Validator]
/// in `form.dart`. The package is one dependency-free Dart file, so it is
/// bundled here instead to keep shadcn_flutter's dependency list small.
///
/// Upstream: https://pub.dev/packages/email_validator (v3.0.0), MIT,
/// Copyright (c) Fredrik Eilertsen. The full license text sits next to this
/// file in `LICENSE`. Kept deliberately close to upstream so it stays easy to
/// diff against a newer release.
///
/// This is library-private to shadcn_flutter — it is deliberately not
/// exported from `shadcn_flutter.dart`. The public entry point is the
/// `EmailValidator` [Validator] in `form.dart`.
library;
