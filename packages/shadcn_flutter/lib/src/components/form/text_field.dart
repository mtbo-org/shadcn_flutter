...
class _FocusedToFront extends StatefulWidget {
  ...
  const _FocusedToFront({
    required this.child,
    required this.skipFocusTraversal,
  });
...
class TextFieldState extends State<TextField>
  ...
  /// Mirrors EditableTextState's default (no-onEditingComplete) behavior:
  /// clears the composing region and, for actions that signal the user is
  /// done ("done", "go", "search", "send", "previous", or "newline" on a
  /// single-line field), moves focus away so the on-screen keyboard closes.
  void _handleDefaultEditingComplete() {
    effectiveController.clearComposing();
    // Mirrors EditableTextState's own default action resolution when
    // widget.textInputAction is unset.
    final action =
  ...
  Widget _buildDecorated(BuildContext context) {
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.macOS:
      case TargetPlatform.windows:
        textSelectionControls ??= shadcnTextSelectionHandleControls;
        ...
        child: EditableText(
          ...
          onEditingComplete: () {
            if (widget.onEditingComplete != null) {
              ...
              // EditableText only unfocuses (closing the on-screen keyboard
              // when the "done"/"go"/"search"/"send"/"previous" action is
              // pressed) when its onEditingComplete is null, because it
              // assumes a non-null callback fully owns that responsibility.
              // We always pass a non-null callback here, so replicate that
              // default behavior ourselves; otherwise the keyboard never
              // closes on platforms (notably iOS) that rely on it. See
              // EditableTextState._finalizeEditing.
              _handleDefaultEditingComplete();
            }
            _formatSubmit();
          },
          onSubmitted: (value) {
