...
FormattedValue _dateLikeValue() {
  return const FormattedValue([
    FormattedValuePart(InputPart.editable(length: 2, width: _partWidth), 'AB'),
    FormattedValuePart(InputPart.static('/')),
    FormattedValuePart(InputPart.editable(length: 2, width: _partWidth), 'CD'),
    FormattedValuePart(InputPart.static('/')),
    FormattedValuePart(InputPart.editable(length: 2, width: _partWidth), 'EF'),
  ]);
}
  ...
  group('FormattedInput cross-part selection', () {
    testWidgets('Select All selects every editable part', (tester) async {
      await tester.pumpWidget(
        SimpleApp(child: FormattedInput(initialValue: _dateLikeValue())),
      );
